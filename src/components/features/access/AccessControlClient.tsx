        ...
        <div className="lg:col-span-3 space-y-6">
                 ...
                 {profiles.map((p) => (
                   <div key={p.id} className="relative group">
                     <button 
                      ...
                      className={`w-full p-4 rounded-xl flex items-center transition-all border-2 ${selectedUser?.id === p.id ? 'bg-blue-50 border-[#0a2d4d] shadow-sm' : 'bg-gray-50/30 border-transparent hover:bg-gray-50'}`}
                     >
                        <div className="flex items-center gap-3 text-left overflow-hidden">
                           <div className={`w-10 h-10 rounded-lg flex-shrink-0 flex items-center justify-center font-black text-xs ${selectedUser?.id === p.id ? 'bg-[#0a2d4d] text-white' : 'bg-white text-gray-400 shadow-sm border border-gray-100'}`}>
                     ...
                     {currentUserRole === 'admin' && (
                       <div className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/40 backdrop-blur-sm p-1 rounded-lg">
                          <button onClick={() => setEditModalUser(p)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Editar"><Edit3 size={14} /></button>
                          <button onClick={() => handleDeleteUser(p.id, `${p.first_name} ${p.last_name}`)} disabled={deletingId === p.id} className="p-1.5 text-red-600 hover:bg-red-100 rounded disabled:opacity-50" title="Eliminar">{deletingId === p.id ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}</button>
                       </div>
                     )}
                   </div>
                 ))}
           ...
           {currentUserRole === 'admin' && (
              <button 
                ...
                className="w-full py-4 bg-white border border-gray-200 rounded-xl flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest text-[#0a2d4d] hover:bg-[#0a2d4d] hover:text-white transition-all shadow-sm"
              >
                <UserPlus size={16} /> Registrar Nuevo Usuario
              </button>
           )}
        </div>
           ...
           <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden h-full flex flex-col">
              <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/20">
                 <div className="flex gap-4 items-center">
                    ...
                    <div>
                       <h3 className="text-sm font-black uppercase tracking-widest">MATRIZ DE PERMISOS</h3>
              ...
              <div className="px-8 py-4 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                 <span>Mostrando {departments.length} departamentos</span>
